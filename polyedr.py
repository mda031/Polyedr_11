...
class Polyedr:
    ...
    def draw(self, tk):
        ...
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            # Если список просветов пуст, то значит отрезок невидимый
            if len(e.gaps) == 0:
                # Находим центр невидимого ребра
                c_x = (e.beg.x + e.fin.x) / 2
                c_y = (e.beg.y + e.fin.y) / 2
                c_z = (e.beg.z + e.fin.z) / 2
                # Проверяем, что не лежит внутри единичного куба
                if not ((c_x < 0.5 and c_x > -0.5) and (
                         c_y < 0.5 and c_y > -0.5) and (
                         c_z < 0.5 and c_z > -0.5)):

                    # Проверяем угол с горизонтальной плоскостью
                    if math.asin(abs(e.beg.z - e.fin.z) / math.sqrt((
                                     e.beg.x - e.fin.x)**2 +
                                                      (e.beg.y - e.fin.y)**2 +
                                                      (e.beg.z - e.fin.z
                                                       )**2)) <= math.pi / 7:
                        sum += math.sqrt((e.beg.x -
                                          e.fin.x)**2 +
                                         (e.beg.y -
                                          e.fin.y)**2)
    ...
    # Тот же самый метод draw, но без графики
            ...
            if len(e.gaps) == 0:
                ...
                if not ((c_x < 0.5 and c_x > -0.5) and (
                    ...
                    if math.asin(abs(e.beg.z - e.fin.z) / math.sqrt((
                        ...
                        # Добавляем проекцию данного ребра к переменной sum
                        sum += math.sqrt((e.beg.x -
                                          e.fin.x)**2 +
                                         (e.beg.y -
                                          e.fin.y)**2)
